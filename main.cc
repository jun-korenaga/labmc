...
int main(int argc, char **argv)
{
	...
	std::cerr << "no flow law specified\n";
	nerror++;
    }
    if (nerror){
	std::cerr << "invalid command option[s] - abort\n";
	exit(1);
    }
    if (verbose){
	std::cerr << "the number of parallel flow laws: "
	     << model.numParallel() << '\n';
	std::cerr << "the number of sequential flow laws: "
	     << model.numSequential() << '\n';
    }
    ...
    if (verbose){
	std::cerr << "the number of data: " << ndata << '\n';
	model.printRunIds(std::cerr);
    }
	...
	if (ifix(i)>0 && ifix(i)<=np
	    ...
	    if (verbose){
		std::cerr << model.unfixedParam(ifix(i))->name()
			  << " will be fixed to "
	...
	//
	int k = int(round(distr(gen)*(np-1)+1));
	...
	if (ii%dn_out == 0){
	    char line[MaxStr];
 	    snprintf(line, MaxStr, "%6d %5.3f %5.3f ", ii, chi2/ndata, chi2_orig/ndata);
 	    std::cout << line;

	    for (int i=1; i<=model.numUnfixedParams(); i++){
		snprintf(line, MaxStr, "%6.5e ", model.unfixedParam(i)->value());
		std::cout << line;
	    }

	    for (int i=1; i<=model.numParallel(); i++){
		if (model.parallel(i)->needScaling()){
		    snprintf(line, MaxStr, "%6.5e ", model.parallel(i)->scaling());
		    std::cout << line;
		}
	    }
		...
		if (model.sequential(i)->needScaling()){
		    snprintf(line, MaxStr, "%6.5e ", model.sequential(i)->scaling());
		    std::cout << line;
		}
	    }
