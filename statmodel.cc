...
void StatModel::readData(char *fn)
{
    ...
    std::istream* pin = new std::ifstream(fn);
...
{
    double newv = v + dv*2.0*(distr(gen_data)-0.5);
    return newv;
}

void StatModel::printRunIds(std::ostream& os)
{
    os << "runids: ";
...
void StatModel::setUp()
{
    ...
    Atb1.resize(i_para_scaling.size());
    Atb2.resize(i_seq_scaling.size());
    alpha1.resize(i_para_scaling.size());
    alpha2.resize(i_seq_scaling.size());
    chivec.resize(orig_data.size());
//    misfit_vec.resize(orig_data.size()); // for debug
    ...
    cg_idum = long(-(abs(data_lognorm2*1000)+1)); // for pseudorandom numbers
    data_idum = long(-(abs(data_lognorm2*1000)+2)); // for pseudorandom numbers
    gen_cg.seed(cg_idum);
    gen_data.seed(data_idum);
}
...
bool StatModel::calcBestFitA()
{
    ...
    if (i_para_scaling.size()>0 && i_seq_scaling.size()>0){
	if (!do_CG){
	    std::cerr << "do_CG needs to be set\n"; 
	    exit(1);
	}
    }
	...
	for (int k=1; k<=i_para_scaling.size(); k++){
	    ...
	    Atb1(k) = 0.0;
	    for (int i=1; i<=data.size(); i++){
		Atb1(k) += Fmat1(i,k)*tmp_bvec(i);
	    }
	}

	Array1d<double> xvec(Atb1.size());
	gaussJordan(AtA1,Atb1,xvec);

	// check if there's a negative scaling constant...
	for (int i=1; i<=i_para_scaling.size(); i++){
	    if (xvec(i)<0) return false;
	}
	...
	for (int i=1; i<=i_para_scaling.size(); i++){
	    xvec(i) *= beta/alpha1(i);
	    para(i_para_scaling(i))->setScaling(xvec(i));
	}
    }else{
	...
	for (int k=1; k<=i_seq_scaling.size(); k++){
	    ...
	    Atb2(k) = 0.0;
	    for (int i=1; i<=data.size(); i++){
		Atb2(k) += Fmat2(i,k)*tmp_bvec(i);
	    }
	}

	Array1d<double> xvec(Atb2.size());
	gaussJordan(AtA2,Atb2,xvec);
	...
	for (int i=1; i<=data.size(); i++){
		...
		pre += Fmat2(i,j)*xvec(j);
	    }
	    tmpsum += log(tmp_bvec(i)/pre);
	}
	double beta = exp(tmpsum/data.size());
	for (int i=1; i<=i_seq_scaling.size(); i++){
	    double val = alpha2(i)/(beta*xvec(i));
...
bool StatModel::calcBestFitA_CG()
{
	...
	for (kk=1; kk<=logA.size(); kk++){
	    logA(kk) = minlogA(kk) + (maxlogA(kk)-minlogA(kk))*distr(gen_cg);
	}
	setA(logA);
