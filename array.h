...
//	if (i>size()){cerr << "Array1d OoR " << i << " " << size() << '\n';}
    ...
    T* begin() { return &(std::vector<T>::front()); }
    T* end() { return (&(std::vector<T>::back()))+1; }
    const T* begin() const { return &(std::vector<T>::front()); }
    const T* end() const { return (&(std::vector<T>::back()))+1; }


    void resize(size_t n, const T& val = T()) { std::vector<T>::resize(n, val); }
    void push_back (const T& x) { std::vector<T>::push_back(x); }
    void sort_unique();

    // for Numerical recipes' std::vector
...
protected:
          T& operator[](size_t i) { return std::vector<T>::operator[](i-1); }
    const T& operator[](size_t i) const { return std::vector<T>::operator[](i-1); }
};
...
void Array1d<T>::sort_unique()
{
    ...
    typename std::vector<T>::iterator new_end = unique(tmp.begin(),tmp.end());
...
public:
    Array2d() : std::vector<T>(0) {}
    Array2d(size_t n1, size_t n2)
	: std::vector<T>(n1*n2), nrow(n1), ncol(n2) {}
    ~Array2d();
    
    T&       operator()(size_t i, size_t j)
	{ return std::vector<T>::operator[](offset(i,j)); }
	...
	{ std::vector<T>::resize(n1*n2, val); nrow=n1, ncol=n2; }
...
private:
    ...
    std::vector<T*> datapp;			// for toRecipe();
};
...
public:
    Array3d() : std::vector<T>(0) {}
    Array3d(size_t n1, size_t n2, size_t n3)
	: std::vector<T>(n1*n2*n3), nn1(n1), nn2(n2), nn3(n3) {}
    ~Array3d();
    
    T&       operator()(size_t i, size_t j, size_t k)
	{ return std::vector<T>::operator[](offset(i,j,k)); }
    const T& operator()(size_t i, size_t j, size_t k) const
	{ return std::vector<T>::operator[](offset(i,j,k)); }
    T* begin() { return std::vector<T>::begin(); }
    T* end() { return std::vector<T>::end(); }
    const T* begin() const { return std::vector<T>::begin(); }
    const T* end() const { return std::vector<T>::end(); }
    
    void resize(size_t n1, size_t n2, size_t n3, const T& val = T())
	{
	    std::vector<T>::resize(n1*n2*n3, val);
